...
class CS_Exchange_opportunity extends CI_Controller

{
...

	{
        ...
        $this->load->model("CS_Exchange_opportunity_model");
        ...
        $page = $this->CS_Exchange_opportunity_model->search($page);



        $this->load->view("AdministratorArea/CS_Exchange_opportunity/Index", compact("page"));

	}

	public function edit($cs_exchange_opportunity_id = null)

	{
        ...
        $cs_exchange_opportunitys = $this->cs_exchange_opportunity_model->search();
...

        {
            ...
            if($cs_exchange_opportunity_id != null)

            {

                $cs = $this->cs_exchange_opportunity_model->find($cs_exchange_opportunity_id);
...

            {
                ...
                $config['upload_path'] = FCPATH.'/assets/uploads/cs_exchange_opportunity';
            ...
            $cs_exchange_opportunity_validated = array("code" => "0x0000-00000", "message" => "Server response success. Request process complete with no error.");

            if($cs_exchange_opportunity_validated["code"] == "0x0000-00000")

            {

                $cs_exchange_opportunity_id = $this->cs_exchange_opportunity_model->save($cs);
                $this->response_library->responseJSON($cs_exchange_opportunity_validated["code"], $cs_exchange_opportunity_validated["message"]);
        ...
        $this->load->view("AdministratorArea/CS_Exchange_opportunity/Edit",$data);

	}



    public function delete($cs_exchange_opportunity_id = null)

    {
...

        {
            ...
            $this->cs_exchange_opportunity_model->save($cs);
...

    {
            ...
            $cs = $this->cs_exchange_opportunity_model->search($model_filter);
